...
{
        return nf_register_net_hooks(net, sla_ipv4_ops,
                                     ARRAY_SIZE(sla_ipv4_ops));
}

static void __net_exit sla_nf_unregister(struct net *net)
{
        nf_unregister_net_hooks(net, sla_ipv4_ops,
                                ARRAY_SIZE(sla_ipv4_ops));
}
...
/*    err = nf_register_hooks(sla_ipv4_ops, ARRAY_SIZE(sla_ipv4_ops));
    if (err < 0)
    {
...
//    nf_unregister_hooks(sla_ipv4_ops, ARRAY_SIZE(sla_ipv4_ops));
